...
async function checkSTLink(url) {
  try {
    const res = await axiosGet(url, {
      method: "HEAD",
      headers: { "User-Agent": UA_FIREFOX, Referer: "https://streamtape.com/" },
      timeout: 6000,
      maxRedirects: 0,
      validateStatus: () => true,
    });
...
async function extractStreamtape(pageUrl) {
  ...
  try {
    ...
    try {
      ...
      console.error("[ST RESOLVER] Error descargando pagina:", e.message);
      return null;
    }
    ...
    for (const el of scriptElems) {
      ...
      while ((match = regex.exec(scriptContent)) !== null) {
        ...
        let link;
        try {
          // Solo evalua concatenacion/slicing de literales de cadena extraidos por el regex de arriba
        ...
        if (validUrl) {
          console.log("[ST RESOLVER] URL resuelta:", validUrl);
          return validUrl;
        }
      }
    }

    console.log("[ST RESOLVER] No se encontro ningun enlace valido");
    return null;
  } catch (e) {
