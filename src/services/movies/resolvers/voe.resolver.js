...
// Decodifica el JSON ofuscado que el propio proveedor de video VOE incrusta en su pagina
// (tecnica estandar de este proveedor, no relacionada a nada agregado por nosotros)
function decodeObfuscatedData(obfuscated) {
  try {
    ...
    console.error("[VOE Resolver] Error decodificando datos:", err.message);
    return null;
  }
}
...
async function extractVoe(pageUrl) {
  ...
  try {
    ...
    try {
      ...
      console.error("[VOE RESOLVER] Error descargando pagina:", e.message);
      return null;
    }
    ...
    if (!script) {
      console.log("[VOE RESOLVER] No se encontro el script JSON ofuscado");
      return null;
    }

    const data = decodeObfuscatedData($(script).html());
    if (!data) {
      console.log("[VOE RESOLVER] Fallo decodificacion de datos");
      return null;
    }

    const finalUrl = data.direct_access_url || data.source || null;
    if (finalUrl) {
      console.log("[VOE RESOLVER] URL resuelta:", finalUrl);
    }
    return finalUrl;
  } catch (err) {
