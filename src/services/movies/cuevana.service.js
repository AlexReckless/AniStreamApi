const { scrapeWithPage } = require("./browser");
  ...
  return scrapeWithPage(url, () => {
    ...
    elements.forEach((el) => {
      ...
      if (linkEl && titleEl) {
        ...
        const rawSlug = href.split("cuevana3.to/").pop();
        ...
        if (poster && poster.startsWith("/")) poster = "https://ww9.cuevana3.to" + poster;
...
async function getContentInfo(slug, type = "movie") {
  ...
  const result = await scrapeWithPage(url, () => {
    ...
    const isSeriesPage =
      currentUrl.includes("/serie/") || currentUrl.includes("/anime/") || document.querySelectorAll('a[href*="/episodio/"]').length > 0;
    ...
    function guessServerKey(name, embedUrl) {
      ...
      if (serverKey.includes("vidhide") || embedUrl.includes("filelions")) return "vidhide";
      return serverKey;
    }
    ...
    if (!isSeriesPage) {
      ...
      languageMenus.forEach((langMenu) => {
        ...
        serverLiList.forEach((serverLi) => {
          ...
          if (correspondingDiv) {
            ...
            if (embedUrl) {
              servers.push({ name: serverName, server: guessServerKey(serverName, embedUrl), language: langName, embedUrl });
            }
          }
          iframeIndex++;
        });
      ...
      info.seasons = [];
      const episodeLinks = Array.from(document.querySelectorAll("a"))
      ...
      episodeLinks.forEach((ep) => {
        ...
        if (match) {
          ...
          seasonsMap.get(seasonNum).push({ number: episodeNum, title: `Episodio ${episodeNum}`, url: ep.href, season: seasonNum });
        }
      });

      for (const [seasonNum, episodes] of seasonsMap.entries()) {
        episodes.sort((a, b) => a.number - b.number);
        info.seasons.push({ number: seasonNum, name: `Temporada ${seasonNum}`, episodes });
      }
...
async function getEpisodeServers(serieSlug, seasonNumber, episodeNumber) {
  ...
  return { serieSlug, season: Number(seasonNumber), episode: Number(episodeNumber), servers: result.servers, url };
}
