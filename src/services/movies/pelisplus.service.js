...
async function searchContent(query) {
  ...
  $("a.Posters-link").each((_, element) => {
    ...
    if (slug) {
      results.push({ id: slug, slug, title, poster, rating, type, url: getAbsoluteUrl(href) });
    }
  });
...
async function getCatalog(type = "movie", genre = "", page = 1, premieres = false) {
  let path = "";
  if (premieres) {
    path = type === "series" ? "/series/estrenos" : "/peliculas/estrenos";
  } else if (genre) {
    path = `/generos/${genre}`;
  } else {
    switch (type) {
  ...
  $("a.Posters-link").each((_, element) => {
    ...
    if (slug) {
      items.push({ id: slug, slug, title, poster, rating, type: contentType, url: getAbsoluteUrl(href) });
    }
  });

  const hasNextPage =
    $(".pagination a[rel='next']").length > 0 || $(".pagination .page-item:last-child:not(.disabled) a").length > 0;

  return { items, page: Number(page), hasNextPage };
}
...
async function getContentInfo(slug, type = "movie") {
  ...
  $("a[href^='/generos/']").each((_, el) => {
    ...
    if (text) genres.push({ name: text, slug: gSlug });
  });
    ...
    if (seasonsTab.length > 0) {
      seasonsTab.each((_, el) => {
        ...
        nextUl.find("a").each((_, epLink) => {
          ...
          if (epSlugMatch) {
            ...
            episodes.push({ number: epNum, title: epText || `Episodio ${epNum}`, url: getAbsoluteUrl(epHref), season: seasonNum });
          }
        });

        episodes.sort((a, b) => a.number - b.number);
        contentInfo.seasons.push({ number: seasonNum, name: seasonTitle || `Temporada ${seasonNum}`, episodes });
      });
      ...
      $("a[href*='/temporada/']").each((_, epLink) => {
        ...
        if (match) {
          ...
          if (!episodesMap.has(sNum)) episodesMap.set(sNum, []);
          episodesMap.get(sNum).push({ number: eNum, title: epText || `Episodio ${eNum}`, url: getAbsoluteUrl(epHref), season: sNum });
        }
      });

      for (const [seasonNum, episodes] of episodesMap.entries()) {
        episodes.sort((a, b) => a.number - b.number);
        contentInfo.seasons.push({ number: seasonNum, name: `Temporada ${seasonNum}`, episodes });
      }
      contentInfo.seasons.sort((a, b) => a.number - b.number);
    }
  }
...
async function getEpisodeServers(serieSlug, seasonNumber, episodeNumber) {
  ...
  return { serieSlug, season: Number(seasonNumber), episode: Number(episodeNumber), title, servers, url };
}
...
function parseServers($) {
  ...
  if ($("#link_url span").length > 0) {
    ...
    $("#link_url span").each((_, spanEl) => {
      ...
      if (embedUrl) {
        servers.push({ name, server: guessServerKey(name, embedUrl), language: generalLanguage, embedUrl });
      }
    });
  }
  ...
  if (servers.length === 0) {
    $("li.playurl").each((_, el) => {
      ...
      if (embedUrl) {
        servers.push({ name, server: guessServerKey(name, embedUrl), language, embedUrl });
      }
    });
  }
...
function guessServerKey(name, embedUrl) {
  let serverKey = name.toLowerCase().replace(/[^a-z0-9]/g, "");
  if (serverKey.includes("streamwish") || embedUrl.includes("streamwish")) return "streamwish";
  if (serverKey.includes("voesx") || serverKey.includes("voe") || embedUrl.includes("voe.sx")) return "voesx";
  if (serverKey.includes("streamtape") || embedUrl.includes("streamtape")) return "streamtape";
  if (serverKey.includes("netu") || serverKey.includes("hqq") || embedUrl.includes("hqq") || embedUrl.includes("waaw") || embedUrl.includes("netu")) return "netu";
  if (serverKey.includes("vidhide") || embedUrl.includes("vidhide")) return "vidhide";
  return serverKey;
}
...
async function getGenres() {
  ...
  $("a[href^='/generos/']").each((_, el) => {
    ...
    if (text && !genres.some((g) => g.slug === slug)) genres.push({ name: text, slug });
  });
